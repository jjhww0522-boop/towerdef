...
try {
  for (const viewport of [{width:390,height:844},{width:360,height:640},{width:667,height:375}]) {
    ...
    await action(page,'#summon-btn'); await page.locator('#tutorial-guide[data-step=skirmish]').waitFor();
    tick(room,40); await page.waitForFunction(()=>window.tutorialField.player.enemies.length>0);
    assert.ok(room.game.players[0].enemies.some(e=>e.hp<e.maxHp));
    await page.screenshot({path:`artifacts/tutorial-${viewport.width}-first-defense.png`});
    tick(room,40); await page.locator('#tutorial-guide[data-step=inspect]').waitFor();
    assert.ok(room.game.players[0].enemies.length>=2);
    const frozen = room.game.players[0].enemies.map(e=>[e.id,e.hp,e.progress]);
    tick(room,100); await page.waitForTimeout(300);
    assert.deepEqual(room.game.players[0].enemies.map(e=>[e.id,e.hp,e.progress]),frozen);
    assert.equal(await page.evaluate(()=>window.tutorialField.combatPaused),true);
    await tapRobot(page,anchor.id); await page.locator('#unit-dialog[open]').waitFor();
    ...
    await action(page,'#tutorial-next'); await page.locator('#summon-btn:enabled').waitFor();
    await action(page,'#summon-btn'); await page.locator('#tutorial-guide[data-step=sell]').waitFor();
    assert.equal(room.game.players[0].units.length,3);
    assert.equal(await page.locator('#summon-btn').isDisabled(),true);
    pass(`${viewport.width}: two draws, real opening combat, safe reading pause and 0.7s cooldown`);
    ...
    const saleState=()=>({seq:room.game.players[0].lastSeq,gold:room.game.players[0].gold,ids:room.game.players[0].units.map(u=>u.id)});
    const beforeSale=saleState();
    await page.locator('#sell-unit').tap(); await page.locator('#sale-dialog[open]').waitFor();
    assert.equal(await page.locator('#unit-inspection-dialog').isVisible(),false,'sale needs no management screen');
    assert.equal(await page.locator('[data-close=sale-dialog]').evaluate(el=>el===document.activeElement),true);
    assert.match(await page.locator('#sale-description').innerText(),/사라지고/);
    assert.equal(await page.locator('#sale-unit-name').innerText(),'점화봇');
    await page.locator('[data-close=sale-dialog]').tap(); await page.locator('#sale-dialog').waitFor({state:'hidden'});
    assert.deepEqual(saleState(),beforeSale,'cancelling never sends a sale');
    await page.locator('#sell-unit').tap(); await page.keyboard.press('Escape');
    await page.locator('#sale-dialog').waitFor({state:'hidden'}); assert.deepEqual(saleState(),beforeSale);
    ...
    await page.locator('#tutorial-guide[data-step=counterattack]').waitFor();
    assert.equal(await page.evaluate(()=>window.tutorialField.combatPaused),false);
    tick(room,10); await page.waitForTimeout(300);
    await page.screenshot({path:`artifacts/tutorial-${viewport.width}-counterattack.png`});
    for(let i=0;i<600&&room.game.tutorial.step==='counterattack';i++) tick(room,1);
    assert.equal(room.game.players[0].enemies.length,0);
    assert.equal(room.game.players[0].kills,3);
    ...
    await page.locator('#home-play').tap(); await page.locator('#quick-start:enabled').tap();
    await page.locator('#summon-btn:enabled').waitFor(); await action(page,'#summon-btn');
    const normal=rooms.at(-1).game.players[0], robot=normal.units[0];
    await tapRobot(page,robot.id); await page.locator('#sell-unit:enabled').waitFor();
    const normalBefore={seq:normal.lastSeq,gold:normal.gold,ids:normal.units.map(u=>u.id)};
    await page.locator('#sell-unit').tap(); await page.locator('#sale-dialog[open]').waitFor();
    await page.locator('[data-close=sale-dialog]').tap();
    assert.deepEqual({seq:normal.lastSeq,gold:normal.gold,ids:normal.units.map(u=>u.id)},normalBefore);
    await page.locator('#sell-unit').tap(); await action(page,'#confirm-sale');
    assert.equal(normal.units.length,0); assert.equal(normal.gold,normalBefore.gold+Math.floor(robot.investedGold*rooms.at(-1).game.rules.salvageRefundRatio));
    await page.locator('#sale-dialog').waitFor({state:'hidden'});
    pass(`${viewport.width}: normal expedition sale opens directly, cancels safely and refunds once`);
    await page.locator('#leave-btn').tap(); await context.close();
  }
