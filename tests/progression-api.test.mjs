...
test('profile and research endpoints authenticate authority and never accept client balances', async t => {
  ...
  const created = await request('/profile', { researchCredits: 9000, unlockedRecipes: ['make_liu_bei'], clearedBattlefields: [1, 2, 3] });
...
test('real clears, concurrent research, unlocked deployment and receipts survive profile-server restart', async t => {
  ...
  let session = (await request('/session', { roomId: 'first', playerId: 'one', profileToken: account.profileToken, speed: 1 })).body;
  finish(room);
  const state = (await request('/state', undefined, session.token)).body;
  assert.equal(state.players[0].result.researchCredits, 40);
  assert.equal(state.profile.researchCredits, 40);
  assert.equal(state.profile.stats.clears, 1);
  assert.deepEqual(state.profile.unlockedBattlefields, [1, 2]);
  for (let index = 0; index < 3; index++) assert.equal((await request('/state', undefined, session.token)).body.profile.researchCredits, 40);
  const resumed = await request('/session', { roomId: 'first', playerId: 'one', profileToken: account.profileToken }, session.token);
  assert.equal(resumed.body.profile.researchCredits, 40);
  assert.equal((await request('/research', { recipeId: 'make_liu_bei' }, account.profileToken)).body.error, 'insufficient_research_credits');
  session = (await request('/session', { roomId: 'second', playerId: 'one', profileToken: account.profileToken })).body;
  finish(room);
  assert.equal((await request('/profile', undefined, account.profileToken)).body.profile.researchCredits, 80, 'profile reads settle a completed match too');
  ...
  assert.ok(purchases.every(response => response.body.profile.researchCredits === 20));
  const deployed = (await request('/session', { roomId: 'third', playerId: 'one', battlefieldId: 2, profileToken: account.profileToken })).body;
  assert.equal(deployed.state.battlefieldId, 2);
  assert.deepEqual(deployed.state.players[0].unlockedRecipes, ['make_liu_bei']);
  assert.equal(deployed.state.rules.totalWaves, core.content.battlefields[1].rules.totalWaves);
  ...
  assert.equal(profile.researchCredits, 20);
  assert.equal(profile.stats.clears, 2);
  assert.deepEqual(profile.unlockedRecipes, ['make_liu_bei']);
  assert.equal(restartedStore.recordResult(profile.id, state.expeditionId, state.players[0].result).applied, false);
  assert.equal((await restarted.request('/research', { recipeId: 'make_liu_bei' }, account.profileToken)).body.profile.researchCredits, 20);
});
