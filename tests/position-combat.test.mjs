...
function setup(definitionId = 'shu_guard', slot = 2) {
  const game = createGame({ playerIds: ['one', 'two'], seed: 15 });
  game.rules.spawnIntervalTicks = game.rules.minSpawnIntervalTicks = 99999;
  game.rules.summonCooldownTicks = 0; // Isolate placement/range; real summon timing has separate tests.
...
test('placement visits all empty slots without changing unit draws or leaking RNG state', () => {
  ...
  a.rules.summonCooldownTicks = 0; // Sample placement RNG without advancing combat.
  const b = structuredClone(a); b.placementRngState = 87;
  const counts = Array(a.rules.maxUnits).fill(0);
  ...
  assert.ok(counts.every(count => count > 6000 / counts.length * .7 && count < 6000 / counts.length * 1.35), JSON.stringify(counts));
...
test('summon preserves occupied and dispatched slots, respects the cap and retries exactly once', () => {
  ...
  for (let seq = 2; seq <= game.rules.maxUnits; seq++) {
  ...
  assert.equal(new Set(player.units.map(unit => unit.slot)).size, game.rules.maxUnits);
  assert.equal(player.units.filter(unit => unit.slot === reserved).length, 1);
  assert.equal(applyAction(game, player.id, { seq: game.rules.maxUnits + 1, type: 'summon' }).error, 'UNIT_CAP');
});
...
test('either selected recipe ingredient anchors the result, even at a higher slot number', () => {
  for (const selectedIndex of [0, 1]) {
    ...
    player.units = recipe.ingredients.map((definitionId, i) => ({ id: game.nextEntityId++, definitionId,
      slot: [game.rules.maxUnits - 1, 2][i], dispatched: false, attackCooldownTicks: 0, investedGold: 20 }));
...
test('portrait rotates the same slot topology instead of dealing a new arrangement', () => {
  ...
  const a = projectPoint(unitPoint(0), road, true), b = projectPoint(unitPoint(4), road, true);
  close(a.x, b.x); assert.ok(a.y < b.y);
  const c = projectPoint(unitPoint(5), road, true);
